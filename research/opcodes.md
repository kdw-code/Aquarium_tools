...
|              |                  |                    | If `Operand` is -1  it means the index point to an array element then `R0` will contain the `index` of the destination variable and `R1` will contains the `value` or the `index to the string` |
...
|              |                  |                    | If `Operand` is -1 it means the index point to an array element then `R0` will contain the `index` of the variable |
|              |                  |                    | if the next instruction is a LOAD instruction it's a Preincrement, and else if it's a POP instruction it's a Postincrement |
|              |                  |                    | If `Operand` is positive then the operand will be the `index` of the variable |
|              |                  |                    | if the next instructions are VAL and LOAD it's a Preincrement and else if they are VAL and POP instruction it's a Postincrement |
| 0x23         | Dec              | Index              | Decrement the variable at index by 1 |
|              |                  |                    | If `Operand` is -1 it means the index point to an array element then `R0` will contain the `index` of the variable |
|              |                  |                    | if the next instruction is a LOAD instruction it's a Predecrement, and else if it's a POP instruction it's a Postdecrement |
|              |                  |                    | If `Operand` is positive then the operand will be the `index` of the variable |
|              |                  |                    | if the next instructions are VAL and LOAD it's a Predecrement and else if they are VAL and POP instruction it's a Postdecrement |
...
| 0x31         | Assign_sub           | Index              | Subtract `value` to variable located at `index`  |
...
|              |                  |                    | `int_var = GET_INT_VARIABLE(index);` |
|              |                  |                    | `int_var ^= value` |
| 0x38         | Assign_sar           | Index              | Shift arithmetic right `value` to variable at `index` |
|              |                  |                    | if operand == -1, it means the index point to an array element `R0` contains the `index` and `R1` contains the `value` |
|              |                  |                    | Else `operand` contains the `index` and `R0` contains the `value` |
|              |                  |                    | `int_var = GET_INT_VARIABLE(index);` |
|              |                  |                    | `int_var >>= value` |
| 0x39         | Assign_shl           | Index              | Shift logical left `value` to variable at `index` |
|              |                  |                    | if operand == -1, it means the index point to an array element `R0` contains the `index` and `R1` contains the `value` |
|              |                  |                    | Else `operand` contains the `index` and `R0` contains the `value` |
|              |                  |                    | `int_var = GET_INT_VARIABLE(index);` |
|              |                  |                    | `int_var <<= value` |
| 0x3A         | Assign_shr           | Index              | Shift logical right `value` to variable at `index` |
|              |                  |                    | if operand == -1, it means the index point to an array element `R0` contains the `index` and `R1` contains the `value` |
|              |                  |                    | Else `operand` contains the `index` and `R0` contains the `value` |
...
| 0x46         | Continue         | Depth              | Continue from a control flow structure at x depth, this instruction don't appear in release binary |
|              |                  |                    | and is converted to a JMP instead |
